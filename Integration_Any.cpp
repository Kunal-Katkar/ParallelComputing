...
double tan_reduction(int power,double lower_limit,double upper_limit)
{
	...
	if(power==0)
		...
		return (((1.0/(power-1))*pow(tan(upper_limit),power-1))-((1.0/(power-1))*pow(tan(lower_limit),power-1))+tan_reduction(power-2,lower_limit,upper_limit));
}
...
int main(int argc,char* argv[])
{
  ...
  MPI_Bcast(&tan_highest_coefficient,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
	...
	MPI_Bcast(sine_coefficients,26,MPI_DOUBLE,0,MPI_COMM_WORLD);
